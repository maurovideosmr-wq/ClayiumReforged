...
- Keep baked Clay Work Table crafting tools (`clay_rolling_pin`, `clay_slicer`, `clay_spatula`) as durable `ClayCraftingToolItem`s with legacy Work Table use semantics: every valid tool-assisted button press consumes one durability, and a tool at max damage is replaced by its configured clay ball remainder after that accepted press.
