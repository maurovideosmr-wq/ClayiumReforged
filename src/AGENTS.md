...
- Register objects through `DeferredRegister` registries in `dev.clayium.clayium.registry`, with Phase 2+ content driven by `ClayiumContentCatalog`.
- Register new items through `ClayiumItems` helpers. Material parts must go through `ClayMaterial`, `ClayPartType`, catalog `MaterialFormSpec` rows, and `PartRegistry`; keep common static aliases for frequently used parts.
- Register Clayium tool behavior through catalog `SimpleItemKind`, `ClayiumItems` factory helpers, `ClayiumTags`, and datagen tag providers. Do not add new tool behavior as a plain `registerSimpleItem` skin.
- Keep baked Clay Work Table crafting tools (`clay_rolling_pin`, `clay_slicer`, `clay_spatula`) as durable `ClayCraftingToolItem`s with legacy Work Table use semantics adapted to 26.x item components: every valid tool-assisted button press consumes one durability, and the press that would reach max damage returns the configured clay ball remainder instead of creating a max-damage tool stack.
- Keep creative tab output grouped by catalog categories: block items, tools, progression items, and material parts.
...
- Any newly added source asset or data resource must be covered by deterministic datagen or a test before the phase closes. New JSON for blockstates, item definitions, models, lang, loot, tags, and cataloged recipes should be generated by `runData` instead of copied by hand.
