...
@ChestContainer
public class ContainerNormalInventory
extends ContainerTemp {
    ...
    public ContainerNormalInventory(InventoryPlayer player, INormalInventory tile, Block block) {
        super(player, new InventoryMultiPage(tile), block, new Object[0]);
    }
    ...
    public void setMachineInventorySlots(InventoryPlayer player) {
        ...
        for (int j = 0; j < tile.getInventoryY(); ++j) {
            for (int i = 0; i < tile.getInventoryX(); ++i) {
                this.addMachineSlotToContainer(new Slot((IInventory)tile, tile.getInventoryStart() + i + j * tile.getInventoryX(), i * 18 + (this.machineGuiSizeX - 18 * tile.getInventoryX()) / 2 + 1, j * 18 + offsetY));
            }
        }
    }
    ...
    public boolean isLargeCallback() {
        return !this.drawPlayerInventoryName();
    }
}
