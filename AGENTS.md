...
- GameTests may be registered through data files/datagen under `data/<namespace>/test_instance` and related test environment/structure resources. Verify the current 26.x API before using programmatic GameTest registration; `RegisterGameTestsEvent` is not present in the selected 26.1.2 artifact.
