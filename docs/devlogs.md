...
## 2026-06-17 Catalog Tier Tooltip Hotfix

- Current goal: restore legacy-style tier tooltip lines for current Clayium catalog items, tools, material forms, and block items without scattering hard-coded tooltip strings.
- Changed files: `src/main/java/dev/clayium/clayium/item/**`, `src/main/java/dev/clayium/clayium/registry/ClayiumContentCatalog.java`, `src/main/java/dev/clayium/clayium/registry/ClayMaterial.java`, `src/main/java/dev/clayium/clayium/registry/MachineHullTier.java`, `src/main/java/dev/clayium/clayium/registry/ClayiumItems.java`, `src/main/java/dev/clayium/clayium/registry/PartRegistry.java`, `src/main/java/dev/clayium/clayium/data/ClayiumLanguageProvider.java`, `src/test/java/dev/clayium/clayium/registry/ClayiumContentCatalogTest.java`, `src/AGENTS.md`, `docs/phase2-content-catalog.md`, `docs/legacy-port-ledger.md`, and generated lang resources after datagen.
- Key decisions: preserve old tier sources instead of inferring only from display names: compressed clay blocks use old metadata tier values, machine hulls use `MachineHullTier`, material forms use `ClayMaterial`, and old `itemMisc` progression rows use their legacy item tiers. Clay tools use the clay material tier so current tool rows are consistently readable in-game.
